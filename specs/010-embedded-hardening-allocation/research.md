...
- **Outcome (2026-06-19): DEFERRED.** Code inspection of `message_handler.rs` confirmed the per-request `tokio::spawn` (via the `async_service_call!` macro → `HandleMessageResult::AsyncMessage(JoinHandle)`) is the **request-level panic-isolation boundary**: a panicking request resolves to a `JoinError` and the connection task survives. An inline fast-path would have to poll the node-manager Read future in the connection task (`now_or_never`-style), which (a) removes that isolation — a node-manager panic would propagate into connection handling, in tension with Constitution IV and the just-landed US1 panic-surface hardening — and (b) only helps the narrow, data-dependent subset of Reads that are immediately ready (node-manager Read is genuinely async and may perform I/O). The measured SC-004 win is already banked: event-path pooling makes per-publish-tick allocation **constant** (2 allocs at both 1000 and 5000 events). Per Constitution I (correctness over completion) and II (do it right once), the isolation risk is not justified by an unmeasured, narrow saving. Recommend a separate, properly-benchmarked effort if a future profile shows dispatch spawn to be a real steady-state hotspot.
...
- **Outcome (2026-06-19): DEFERRED / staged.** Code inspection of `encoding.rs` confirmed both decode traits are defined as `fn decode<S: std::io::Read + ?Sized>(stream: &mut S, ...)` — the entire decode surface (every generated and hand-written `BinaryDecodable`/`SimpleBinaryDecodable`, plus any downstream implementor of these public traits) is generic over an arbitrary byte *stream*, not an in-memory `Bytes`. `ByteString`/`String`/array decode `read_exact` into a freshly-sized `Vec`. Making these slice from a shared buffer requires either (a) a **breaking change to the public decode trait signatures** (carry a `Bytes`/`Buf`), propagated across hundreds of impls and breaking external implementors, or (b) a duplicated parallel `Bytes`-aware trait for every type. Decisively, OPC UA messages are **chunked**: the decode stream is a reassembly of multiple `MessageChunk`s, so a single contiguous `Bytes` to slice from does not exist for any multi-chunk message — the zero-copy win would apply only to single-chunk messages even after the invasive change. The genuine decode-path allocation DoS (attacker-controlled `value_rank` driving an 8.45 GB `resize`) was found by the US1 fuzz pass and **already fixed**. Per Constitution I (correctness over completion) and II (do it right once), the breaking, narrowly-beneficial change is not justified within this feature. **SC-006 is therefore staged, not met in 010** — recorded transparently rather than forced. A future effort could revisit this as a deliberate, benchmarked trait redesign (e.g. a `Buf`-based decode path) if profiling shows decode copies to be a real steady-state cost.
