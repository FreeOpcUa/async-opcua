...
- [ ] T012 [codex] In `async-opcua-pubsub/src/security/codec.rs`, generate a fresh MessageNonce per encode = `Random[4]` (via `opcua_crypto::random::bytes`) ‖ NetworkMessage `SequenceNumber` (UInt32); increment the sequence per message within a key epoch and reset to 1 on SecurityTokenId/key change; derive the counter block from it. Add the `// ponytail:` ceiling comment (IV-unique while seq doesn't wrap a key epoch; key rotation resets it).
...
- [ ] T016 [codex] Extend the interop harness for SignAndEncrypt: either add a secured-NetworkMessage path to `dotnet-tests/external-tests` `pubsub_tests.rs` (currently plaintext UADP only) and/or an open62541 path under `3rd-party/open62541`; if a live harness can't run in CI, produce and commit external known-answer fixtures (raw secured NetworkMessage bytes + key material) under `async-opcua-pubsub/tests/fixtures/`.
