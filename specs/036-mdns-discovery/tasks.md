...
- [X] T016 [P] [US2] [Claude] Integration test (multicast-TOLERANT): a responder server + a querier server; `FindServersOnNetwork` on the querier returns the responder with correct URL + caps; SKIPS / soft-passes when multicast is unavailable (Spec: SC-002)
