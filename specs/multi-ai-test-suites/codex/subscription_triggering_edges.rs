//! Candidate integration tests for async-opcua/tests/integration/subscriptions.rs.
//!
//! To run inside the main suite, copy this file into `async-opcua/tests/integration/`
//! and add `mod subscription_triggering_edges;` to `mod.rs`. It intentionally uses
//! the existing integration-test fixture and does not start the demo interop server.
...
async fn set_triggering_delivers_queued_sampling_item() {
    ...
    for _ in 0..2 {
        let (rv, dv) = timeout(Duration::from_secs(2), data.recv())
            ...
            .unwrap()
            .unwrap();
        by_node.insert(rv.node_id, dv.value);
    }
    assert_eq!(by_node.get(&trigger_id), Some(&Some(Variant::Int32(1))));
    assert_eq!(by_node.get(&linked_id), Some(&Some(Variant::Int32(101))));
}
